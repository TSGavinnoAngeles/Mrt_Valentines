...
import ReactFlow, { Node,addEdge, Background,Edge,Connection,useNodesState,useEdgesState,Controls,updateEdge} from "reactflow";
...
const BasicFlow = () => {
  ...
  const [changeHndl, setchangeHndl] = useState<number[]>([]);
...
useEffect(() => {
  ...
  const updateStationCoords = async () => {
    try {
      const response = await fetch(`http://localhost:0905/stations/conordinates/${connId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateOpsData = async () => {
    try {
      const response = await fetch(`http://localhost:0905/operation/65ca7c3a80dc6dceeb1b8f67`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
const buttonHandler = () => {
  ...
  const handleOrigin = (arr: number[]) => { 
    setchangeHndl(arr)
    console.log(changeHndl)
  }
  function handleLeafletChange(index: number, e: LeafletEvent,) {
    const locations  = (e.target.getLatLng())

    setCoordinates2([locations.lng, locations.lat]);
...
<div> 
  ...
  <div className= "flex mr-2 bg-train">
    {isClicked === false ? (
        ...
        {open === true ? (
            ...
            <div style={{ height: '100%', width: '100%' }} className="w-full px-2 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-white-800 dark:border-gray-700 align-right max-h-80 overflow-y-auto my-2">
                <MapContainer center={[14.6522, 121.0323]} zoom={13} dragging={true} scrollWheelZoom={true} style={{ height: '100%', width: '100%' }}>
                  <TileLayer
                    attribution='Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ'
                    url="https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}"
                  ...
                  <MapComponent setLatClicked={setLatClicked} setLngClicked={setLngClicked} />
        ...
        ( <>
                       <div className = "text-xl">Details of Operation</div>
                        ...
                        <form className="w-1/4 px-2 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-white-800 dark:border-gray-700 align-right m-2">
                          <div className=" m-2 flex flex-wrap -mx-3 mb-6 ">
                              ...
                              onChange={(e) =>{setFare(Number(e.target.value))} }
                                value={fare}
                                className="appearance-none block w-full bg-gray-200 text-gray-700 border border-black-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" type="text"  />
                              </div>
                              <div className="w-full  px-3">
                                <label className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="grid-last-name">
                        ...
                        <button
                          ...
                          onClick={baseFareButtonHandler}> Edit Fare
                        </button> 
                        </form>
                        ...
                        <form className="w-3/4 px-2 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-white-800 dark:border-gray-700 align-right mt-2">
                          ...
                          {coords.map((c) => {
                            ...
                            if (c._id === connId) {
                              return (
                                <div style={{ height: '280px', width: '100%' }} className="w-full px-2 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-white-800 dark:border-gray-700 align-right max-h-80 overflow-y-auto my-2">  
                                <MapContainer center={[c.lat, c.lng]} zoom={13} dragging= {true} scrollWheelZoom={true} style={{height:'100%', width: '100%'}}>
                                 <TileLayer attribution='Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ'
                                    url="https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}"
                                    maxZoom={16}
                                  />
                                  {coords.map((cin, index) => (  
                                    // handleOrigin(cin.lat, cin.lng)
                                    ...
                                    cin._id === connId ? (
                                      ...
                                      eventHandlers={{
                                        moveend: (e) => {
                                              handleLeafletChange(index, e)
                                              // console.log(coordinates2)
                                        }
                                      }}
                                      key={index}
                                      position={[cin.lat, cin.lng]}
                                      draggable={true}>
                                    <Popup>{c.label}</Popup>
                                    </Marker> 
                                      ...
                                      position={[cin.lat, cin.lng]}
                                      draggable={false}>
                                    <Popup>{cin.label}</Popup>
                                    </Marker> 
                                    )                        
                                  ))}
                                  {coords.map((station, index) => (
                                    station.connecting.map((cin) => {
                                      const connsStat = coords.find((s) => s._id === cin);
                                      if (connsStat) {
                                        return (
                                          <Polyline
                                            key={`${index}-${connsStat._id}`}
                          ...
                          (<>
                          <div style={{ height: '280px', width: '100%' }} className="w-full px-2 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-white-800 dark:border-gray-700 align-right max-h-80 overflow-y-auto my-2">  
                              <div className = "text-xl"> Click on a node in the connections view to edit the location of the station. The station you'll be editing will appear to be gray. Afterwards save your edits in the rightmost button below. Or to delete a station flag your station for deletion by double clicking on the node and then press backspace.</div>
                          </div>
                          </>)}
         </>)}


                        <div className="fixed bottom-0 left-0 right-0 mb-1 mx-4 flex justify-right z-10"> <div> 
                      <label className="relative inline-flex items-center me-5 cursor-pointer algin-right">
                        ...
                        <button onClick ={()=>setOpen(!open)} className="z-10 bg-black hover:bg-green-500 text-white font-semibold hover:text-white py-2 px-4 border border-black hover:border-transparent rounded me-4">
                                      {creationHandler()}
                        </button>
